...
\begin{figure}
\begin{tikzpicture}
    \begin{axis}[axis lines = left, xlabel= Password lengths $x$, ylabel=Number of mappings in $\sigma$, ymin=0]
    
    \addplot[fill=red!50, opacity=0.5, domain=0:30, samples=20]{(100*x)^(2/3)} -| (current plot begin);
    \addlegendentry[red]{$100 <  \frac{y^{1.5}}{x}$};

    \addplot[fill=green!50, opacity=0.5, domain=0:30, samples=20]{(50*x)^(2/3)} -| (current plot begin);
    \addlegendentry[green]{$50 <  \frac{y^{1.5}}{x}$};

    \addplot[fill=blue!50, opacity=0.5, domain=0:30, samples=20]{(20*x)^(2/3)} -| (current plot begin);
    \addlegendentry[blue]{$20 <  \frac{y^{1.5}}{x}$};
\end{axis}
\end{tikzpicture}
