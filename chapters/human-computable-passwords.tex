...
\end{axis}
\end{tikzpicture}
\caption{Number of challenge-response pairs required to start recovering mapping $\sigma$ as a function of the number of mappings in the mapping $n$. }
\label{trade-off1}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Number of accounts needed to start recovering the mapping $\sigma$, given different values of $n$.}
\label{trade-off2}
\end{figure}
...
\begin{figure}
\begin{tikzpicture}
    \begin{axis}[axis lines = left, xlabel= Password length $x$, ylabel=Number of mappings needed in $n$, ymin=0, ymax=200]
...
\begin{definition}[ {\cite{hcp-blocki}} ]
...
\begin{requirement}[ {\cite{naturally-rehearsing}} ]\label{CR}
    ...
    %The rehearsal schedule given by $R(\hat c, i) = i s$ is sufficient to maintain the association $(\hat c, \hat a)$ in memory.
\end{requirement}

\begin{requirement}[ {\cite{naturally-rehearsing}} ]\label{ER}
    ...
    %The rehearsal schedule given by $R(\hat c, i)=2^{i s}$ is sufficient to maintain the association $(\hat c, \hat a)$ in memory.
\end{requirement}
